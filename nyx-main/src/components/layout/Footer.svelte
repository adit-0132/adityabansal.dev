<script lang="ts">
	...
	async function fetchViewCount() {
		const { enabled, abacusInstance, abacusNamespace, abacusKey } = site.brand.analytics;
		if (!enabled || !abacusInstance || !abacusNamespace || !abacusKey) return;
		try {
			const res = await fetch(`${abacusInstance}/hit/${abacusNamespace}/${abacusKey}`, {
				signal: AbortSignal.timeout(2000)
			});
			const data = await res.json();
			viewCount = (data.value as number).toLocaleString();
		} catch (err) {
...
<div class="relative m-auto mx-5 mb-5">
	...
	>
		<div class="flex flex-wrap items-center justify-center gap-x-3 gap-y-2 md:justify-start">
			<span class="whitespace-nowrap">© {year} {site.brand.copyrightName}</span>
		...
		<div class="flex flex-wrap items-center justify-center gap-x-3 gap-y-2 md:justify-end">
			...
			{#if viewCount}
				...
				<span class="text-subtext1" title="Page views">{viewCount} views</span>
			{/if}
